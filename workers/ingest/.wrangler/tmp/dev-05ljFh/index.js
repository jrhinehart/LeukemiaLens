...
// .wrangler/tmp/bundle-cVodSh/checked-fetch.js
...
// .wrangler/tmp/bundle-cVodSh/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-cVodSh/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-cVodSh/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
function extractMetadata(text3) {
  const mutations = [];
  const topics = [];
  ...
  if (/\b(AML|Acute Myeloid Leukemia)\b/i.test(text3)) {
    diseaseSubtypes.push("AML");
  }
  if (/\b(CML|Chronic Myeloid Leukemia)\b/i.test(text3)) {
    diseaseSubtypes.push("CML");
  }
  if (/\b(ALL|Acute Lymphoblastic Leukemia)\b/i.test(text3)) {
    diseaseSubtypes.push("ALL");
  }
  if (/\b(CLL|Chronic Lymphocytic Leukemia)\b/i.test(text3)) {
    diseaseSubtypes.push("CLL");
  }
  if (/\b(MDS|Myelodysplastic Syndromes?)\b/i.test(text3)) {
    diseaseSubtypes.push("MDS");
  }
  const hasComplexKaryotype = /\b(complex karyotype|complex cytogenetics)\b/i.test(text3) || /(>=|greater than or equal to)\s*3\s*abnormalities/i.test(text3);
  const transplantContext = /\b(HSCT|stem cell transplant|bone marrow transplant|allogeneic|autologous)\b/i.test(text3);
  return {
    mutations,
    topics,
    diseaseSubtypes,
...
// src/index.ts
var SEARCH_TERM = '(Leukemia[Title/Abstract]) AND ("2023/01/01"[Date - Publication] : "3000"[Date - Publication])';
var MAX_RESULTS = 20;
var EMAIL = "antigravity@gemini.google.com";
async function searchPubmed(termOverride = null, limit = MAX_RESULTS) {
...
__name(fetchDetails, "fetchDetails");
var src_default = {
  ...
  async processIngestion(env, searchTermOverride = null, limit = MAX_RESULTS) {
    ...
    try {
      ...
      const $2 = load(xmlContent, { xmlMode: true });
      const articles = $2("PubmedArticle");
      for (const article of articles) {
        const pmid = $2(article).find("PMID").first().text();
        const articleNode = $2(article).find("MedlineCitation > Article");
        ...
        const pubDate = `${pubDateYear}-01-01`;
        ...
        articleNode.find("AuthorList > Author").each((_, el) => {
          const last2 = $2(el).find("LastName").text();
          const initials = $2(el).find("Initials").text();
          if (last2)
            authorsList.push(`${last2} ${initials}`);
          const aff = $2(el).find("AffiliationInfo > Affiliation").text();
        ...
        try {
          ...
          await env.DB.prepare("INSERT OR IGNORE INTO links (study_id, url, link_type) VALUES (?, ?, ?)").bind(studyId, `https://pubmed.ncbi.nlm.nih.gov/${pmid}/`, "pubmed").run();
        } catch (e) {
