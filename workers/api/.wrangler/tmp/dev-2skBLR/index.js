...
// .wrangler/tmp/bundle-TekTpU/checked-fetch.js
...
// .wrangler/tmp/bundle-TekTpU/strip-cf-connecting-ip-header.js
  ...
  try {
    ...
    if (mutationsRes.results) {
      mutationsRes.results.forEach((m) => {
        if (!mutationsMap[m.study_id])
          mutationsMap[m.study_id] = [];
    ...
    if (treatmentsRes.results) {
      treatmentsRes.results.forEach((t) => {
        if (!treatmentsMap[t.study_id])
          treatmentsMap[t.study_id] = [];
...
app.get("/api/export", async (c) => {
  ...
  try {
    ...
    topicsRes.results?.forEach((t) => {
      if (!topicsMap[t.study_id])
        topicsMap[t.study_id] = [];
...
app.get("/api/study/:id", async (c) => {
  ...
  return c.json({
    ...study,
    mutations: mutations.results?.map((m) => m.gene_symbol) || [],
    links: links.results || []
  });
});
var src_default = app;
...
// .wrangler/tmp/bundle-TekTpU/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-TekTpU/middleware-loader.entry.ts
